...
static int rowof(const Prob *p, int dpid);
...
dbload(sqlite3 *db, int dsid, int pid, Prob *p)
{
	...
	s = prep(db, "SELECT kid, cost, gamma FROM parameters WHERE pid = ?");
	sqlite3_bind_int(s, 1, pid);
	if (sqlite3_step(s) != SQLITE_ROW)
		die("no parameter set with pid %d", pid);
	p->kid = sqlite3_column_int(s, 0);
	p->cost = sqlite3_column_double(s, 1);
	p->gamma = sqlite3_column_double(s, 2);
	sqlite3_finalize(s);
	if (p->kid != KERNEL_LINEAR && p->kid != KERNEL_RADIAL)
		die("pid %d has kid %d; only 0 and 1 are known", pid, p->kid);
	if (!(p->cost > 0.0))
		die("pid %d has cost %g; it must be positive", pid, p->cost);
	if (p->kid == KERNEL_RADIAL && !(p->gamma > 0.0))
		die("pid %d has gamma %g; a radial kernel needs it positive",
		    pid, p->gamma);

	p->val = ecalloc((size_t)p->m * p->n, sizeof(*p->val));
	p->y = ecalloc((size_t)p->m, sizeof(*p->y));
	p->w = ecalloc((size_t)p->m, sizeof(*p->w));
	p->c = ecalloc((size_t)p->m, sizeof(*p->c));
	...
	while (sqlite3_step(s) == SQLITE_ROW) {
		...
		if (vid < 0 || vid >= p->n)
			die("dsid %d has vid %d outside 0..%d", dsid, vid,
			    p->n - 1);
		if ((r = rowof(p, sqlite3_column_int(s, 0))) < 0)
	...
	for (i = 0; i < p->m; i++)
		if (p->c[i] <= 0.0)
			die("dpid %d has an upper bound of %g", p->dpid[i],
			    p->c[i]);
}

/*
** Store the result under fresh IDs.  Only the support vectors are kept,
** because a multiplier of zero says nothing about the model.  The whole
** write sits in one transaction, so a failure leaves the database as it
** was.
*/
void
dbsave(sqlite3 *db, int dsid, const Prob *p, const Result *r, int *cid,
       int *bid)
{
	sqlite3_stmt *s;
	int i;
	...
	*bid = nextid(db, "SELECT coalesce(max(bid), 0) + 1 FROM bias");
	...
	s = prep(db,
	         "INSERT INTO coefficients (cid, dsid, dpid, alpha)"
	         " VALUES (?, ?, ?, ?)");
	for (i = 0; i < p->m; i++) {
		if (r->alpha[i] <= 0.0)
			continue;
		...
		sqlite3_bind_int(s, 3, p->dpid[i]);
		sqlite3_bind_double(s, 4, r->alpha[i]);
		run(db, s);
	}
	sqlite3_finalize(s);
	...
	sqlite3_bind_int(s, 2, dsid);
	sqlite3_bind_double(s, 3, r->bias);
	run(db, s);
	sqlite3_finalize(s);
...
/*
** Load the model named by mid, together with the dataset it was trained
** on.  A multiplier that was not stored is zero, since only support
** vectors are kept.
*/
void
dbloadmodel(sqlite3 *db, int mid, Prob *p, Model *mo)
{
	sqlite3_stmt *s;
	int r, dpid;
	...
	dbload(db, mo->dsid, mo->pid, p);
	mo->alpha = ecalloc((size_t)p->m, sizeof(*mo->alpha));

	s = prep(db,
	         "SELECT dpid, alpha FROM coefficients"
	         " WHERE cid = ? AND dsid = ?");
	...
	while (sqlite3_step(s) == SQLITE_ROW) {
		...
		if ((r = rowof(p, dpid)) < 0)
			die("cid %d names dpid %d, which dataset %d does not"
			    " carry",
			    mo->cid, dpid, mo->dsid);
		mo->alpha[r] = sqlite3_column_double(s, 1);
	}
	sqlite3_finalize(s);

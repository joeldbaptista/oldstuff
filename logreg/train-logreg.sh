#!/bin/sh
#
# train-logreg.sh - fit a model and record it.
#
# usage: train-logreg.sh [-v] -t training.sqlite3 -d dsid -p pid
#
# It runs logreg, the C99 solver that sits next to this script, which
# reads dataset dsid under parameter set pid and writes the weights into
# coefficients and the intercept into bias.  This script then records the
...
[ -x "$solver" ] || die "cannot find $solver; run make bin first"
...
out=$("$solver" ${verbose:+$verbose} -t "$db" -d "$dsid" -p "$pid")
cid=$(printf '%s\n' "$out" | sed -n 's/^cid=//p')
bid=$(printf '%s\n' "$out" | sed -n 's/^bid=//p')
isint "$cid" && isint "$bid" || die "logreg returned no coefficient or bias ID"

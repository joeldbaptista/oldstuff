#!/bin/sh
#
# add-parameters.sh - load parameter sets into a training database.
#
...
# per parameter set.  The columns are reg, which may also be spelled rid
# and is 0 for no regulariser, 1 for L1 or 2 for L2; lambda, the strength
# of that regulariser; and comment, which may also be spelled comments
# and may itself contain "|".  Lambda is ignored when reg is 0, but it is
# still stored.
#
...
# fitted against several of them.
#
...
awk -F'[|]' -v first="$pid" -v f="$params" -v out="$tmp/p.csv" '
	...
	!seen_header {
		...
		for (i = 1; i <= NF; i++)
			col[trim($i)] = i
		ck = pick("reg", "rid")
		cc = pick("lambda", "lambda")
		cm = pick("comment", "comments")
		if (!ck || !cc) {
			printf "%s: header needs reg and lambda\n",
			    f > "/dev/stderr"
			bad++
			exit 1
		}
	...
	}
	{
		...
		c = trim($cc)
		if (k != "0" && k != "1" && k != "2") {
			printf "%s:%d: reg must be 0, 1 or 2, found %s\n",
			    f, NR, k > "/dev/stderr"
			bad++
		}
		if (!num(c)) {
			printf "%s:%d: lambda must be a number, found %s\n",
			    f, NR, c > "/dev/stderr"
			bad++
		} else if (k != "0" && c + 0 <= 0) {
			printf "%s:%d: lambda must be positive when reg is %s, found %s\n",
			    f, NR, k, c > "/dev/stderr"
			bad++
		}
		note = ""

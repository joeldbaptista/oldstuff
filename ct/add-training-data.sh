#!/bin/sh
#
# add-training-data.sh - load one dataset into a training database.
#
...
# with no header: field 1 is the class label, which is one of 1, 2, ...,
# N, field 2 is the weight, and fields 3 onwards are the variables,
# filed under vid 0 to n - 1.  Data points are numbered from 0 in the
# order read.  The weight is kept so that a file written for the sibling
# svm project loads here unchanged; build-ct ignores it.
#
...
isint "$m" && [ "$m" -ge 2 ] || die "$meta: m must be an integer above 1"
isint "$n" && [ "$n" -ge 1 ] || die "$meta: n must be a positive integer"

# Check the whole data file before touching the database.  The labels
# must run 1, 2, ..., N with no gap, which is what the tree assumes.
awk -F'[|]' -v n="$n" -v want="$m" -v f="$data" '
	...
	/^[ \t\r]*$/ { next }
	{
		...
		lab = trim($1)
		if (lab !~ /^[0-9]+$/ || lab + 0 < 1) {
			printf "%s:%d: label must be 1, 2, ..., N, found %s\n",
			    f, NR, lab > "/dev/stderr"
			bad++
		} else {
			cls[lab + 0]++
	...
	END {
		...
		if (top < 2)
			printf "%s: holds only class 1; at least 2 are needed\n",
			    f > "/dev/stderr"
		for (k = 1; k <= top; k++)
			if (!(k in cls)) {
				printf "%s: reaches class %d but holds no row of class %d\n",
				    f, top, k > "/dev/stderr"
				bad++
			}
		exit (bad > 0 || seen != want || top < 2)
	}
